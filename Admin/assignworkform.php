...
if (isset($_REQUEST['assign'])) {
  ...
  if (($_REQUEST['request_id'] == "") || ($_REQUEST['request_info'] == "") || ($_REQUEST['requestdesc'] == "") || ($_REQUEST['requestername'] == "") || ($_REQUEST['address1'] == "") || ($_REQUEST['address2'] == "") || ($_REQUEST['requestercity'] == "") || ($_REQUEST['requesterstate'] == "") || ($_REQUEST['requesteremail'] == "") || ($_REQUEST['requestermobile'] == "") || ($_REQUEST['assigntech'] == "") || ($_REQUEST['inputdate'] == "")) {
    ...
    $techId = $_REQUEST['assigntech'];
    $rdate = $_REQUEST['inputdate'];
    $sql = "INSERT INTO assignwork_tb (request_id, request_info, request_desc, requester_name, requester_add1, requester_add2, requester_city, requester_state, requester_zip, requester_email, requester_mobile, assign_date) VALUES ('$rid', '$rinfo','$rdesc', '$rname', '$radd1', '$radd2', '$rcity', '$rstate', '$rzip', '$remail', '$rmobile', '$rdate')";
    if ($conn->query($sql) == TRUE) {
      $lastInsertedId = $conn->insert_id;
      $query = "INSERT INTO technician_request (technicial_id, request_id) VALUES ($techId, $lastInsertedId)";
      $conn->query($query);
...
<?php if (isset($msg)) {
  ...
  <form action="" method="POST">
    ...
    <div class="form-row">
      ...
      <div class="form-group col-md-4">
        <label for="requesterstate">Sector</label>
    ...
    <div class="form-row">
      <div class="form-group col-md-6">
        ...
        <select class="form-control" name="assigntech" id="assigntech">
          ...
          <?php
          //select technician to assign work
          $techSelect = '';
          ...
          if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              // Use the data from the database to construct <option> elements
              $techSelect .= '<option value="' . $row['empid'] . '">' . $row['empName'] . '</option>';
            }
          } else {
            // Handle the case where there are no rows in the result set, if needed.
            $techSelect = '<option value="">No Technicians Found</option>';
          }
          echo $techSelect;
          ?>
        </select>
      ...
      <div class="form-group col-md-6">
        ...
        <input type="date" class="form-control" id="inputDate" name="inputdate" min="<?= date('Y-m-d') ?>">
      </div>
    </div>
